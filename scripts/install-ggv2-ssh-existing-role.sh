...
ssh -t $HOST_STRING "AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID} AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} \\
  ...
  --tes-role-name ${ROLE_NAME} --tes-role-alias-name ${ROLE_ALIAS} \\
  --component-default-user root:root --provision false --setup-system-service true --deploy-dev-tools true"
